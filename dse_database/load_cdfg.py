...
import tempfile
from fix_broken_cdfg_dot import fix_broken_cdfg_dot_and_return_lines
import shutil
...
def preproc():
    for dot_file in tqdm(all_cdfg_files):
        print(dot_file)
        with tempfile.NamedTemporaryFile() as fixed_dot_file:
            fixed_lines = fix_broken_cdfg_dot_and_return_lines(dot_file)
            for line in fixed_lines:
                fixed_dot_file.write(str.encode(line))
            fixed_dot_file.seek(0)
            pickle_file_name = f'{basename(dot_file)}'
            # Deal with file name limit
            if len(pickle_file_name)>=255-8:
                pickle_file_name = pickle_file_name[:-8]
            pickle_file_name += '.pickle'
            p = join(dirname(dot_file), pickle_file_name)
            try:
                g = nx.drawing.nx_pydot.read_dot(dot_file)
            except:
                try:
                    g = nx.drawing.nx_pydot.read_dot(fixed_dot_file.name)
                    shutil.copy(fixed_dot_file.name, dot_file)
                except:
                    print('error handling: {}'.format(dot_file))
                    exit(-1)
            save_pickle(g, p, True)
