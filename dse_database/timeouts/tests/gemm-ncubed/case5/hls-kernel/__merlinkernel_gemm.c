...
static void merlin_memcpy_0(double dst[32][2],int dst_idx_0,int dst_idx_1,double src[4096],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (0 * 32 + dst_idx_0) * 2 + dst_idx_1;
...
void mars_kernel_0_2_node_0_stage_0(int j,int exec,int i,double m1[4096],double m1_buf_0[32][2])
{
...
void mars_kernel_0_2_node_1_stage_0(int j,int exec,merlin_uint_512 m2[512],double m2_buf_0[32][2][64])
{
  
#pragma HLS INLINE OFF
  if (exec == 1) {
    memcpy_wide_bus_read_double_3d_2_64_512(m2_buf_0,(size_t )0,(size_t )0,(size_t )0,(merlin_uint_512 *)m2,(size_t )(j * 2 * 8),sizeof(double ) * ((unsigned long )4034),(size_t )4034L);
  }
}
...
void mars_kernel_0_2_bus(int mars_j,int mars_init,int mars_cond,int mars_i_0,int mars_i_1,int mars_i_2,int *mars_i_col_1,int *mars_k_col_1,double m1[4096],double mars_m1_buf_0_0[32][2],double mars_m1_buf_0_1[32][2],merlin_uint_512 m2[512],double mars_m2_buf_0_0[32][2][64],double mars_m2_buf_0_1[32][2][64],double *mars_mult_1,double prod[4096],double mars_prod_buf_0_1[2],double mars_prod_buf_0_2[2])
{
...
void mars_kernel_0_2_node_2_stage_1(int j,int exec,int i,int *i_col,int *k_col,double m1_buf_0[32][2],double m2_buf_0[32][2][64],double *mult,double prod_buf_0[2])
{
...
/* Original: #pragma ACCEL PARALLEL FACTOR=1 */
{
       *i_col = i * 64;
      double sum = (double )0;
/* Original label: inner:for(k = 0;k < 32;k++) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumfor(int k_sub = 0;k_sub < 2;++k_sub) {#pragma ACCEL PARALLEL#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumk_col =(k * 2 + k_sub) * 64;mult = m1_buf[k][k_sub] * m2_buf[k][k_sub][j_sub];sum += mult;}} */
      double _sum_rdc[2];
...
#pragma HLS array_partition variable=_sum_rdc complete dim=1
{
        int k_sub_0;
        merlinL5:
        for (k_sub_0 = 0; k_sub_0 <= 1; ++k_sub_0) 
...
#pragma HLS unroll
      ...
      merlinL4:
      for (k = 0; k < 32; k++) 
/* Original: #pragma ACCEL PARALLEL reduction=sum FACTOR=2 */
      ...
      }
{
        int k_sub_1;
        merlinL2:
        for (k_sub_1 = 0; k_sub_1 <= 1; ++k_sub_1) 
/* Original: #pragma ACCEL ARRAY_PARTITION OFF */
/* Original: #pragma ACCEL PARALLEL COMPLETE */
{
...
#pragma ACCEL interface variable=m1 depth=4096 max_depth=4096
  ...
  int i_col;
  double mult;
/* Original label: outer:for(i = 0;i < 64;i++) {#pragma ACCEL TILE FACTOR=1middle:for(j = 0;j < 32;j++) {#pragma ACCEL PIPELINEdouble m2_buf[32][2][64];double m1_buf[32][2];double prod_buf[2];memcpy(m1_buf[0],(const void *)(&m1[i * 64]),sizeof(double ) * 64);#pragma HLS array_partition variable=m2_buf cyclic factor = 8 dim=3memcpy_wide_bus_read_double_3d_2_64_512(m2_buf,0,0,0,(merlin_uint_512 *)m2,(j * 2 * 8),sizeof(double ) *((unsigned long )4034),4034L);for(int j_sub = 0;j_sub < 2;++j_sub) {#pragma ACCEL PARALLEL FACTOR=1i_col = i * 64;double sum =(double )0;inner:for(k = 0;k < 32;k++) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumfor(int k_sub = 0;k_sub < 2;++k_sub) {#pragma ACCEL PARALLEL#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumk_col =(k * 2 + k_sub) * 64;mult = m1_buf[k][k_sub] * m2_buf[k][k_sub][j_sub];sum += mult;}}prod_buf[j_sub] = sum;}memcpy((void *)(&prod[i * 64 + j * 2]),(const void *)(&prod_buf[0]),sizeof(double ) * 2);}} */
  merlinL8:
/* Original label: outer:for(i = 0;i < 64;i++) {#pragma ACCEL TILE FACTOR=1middle:for(j = 0;j < 32;j++) {#pragma ACCEL PIPELINEdouble m2_buf[32][2][64];double m1_buf[32][2];double prod_buf[2];memcpy(m1_buf[0],(const void *)(&m1[i * 64]),sizeof(double ) * 64);#pragma HLS array_partition variable=m2_buf cyclic factor = 8 dim=3memcpy_wide_bus_read_double_3d_2_64_512(m2_buf,0,0,0,(merlin_uint_512 *)m2,(j * 2 * 8),sizeof(double ) *((unsigned long )4034),4034L);for(int j_sub = 0;j_sub < 2;++j_sub) {#pragma ACCEL PARALLEL FACTOR=1i_col = i * 64;double sum =(double )0;inner:for(k = 0;k < 32;k++) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumfor(int k_sub = 0;k_sub < 2;++k_sub) {#pragma ACCEL PARALLEL#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumk_col =(k * 2 + k_sub) * 64;mult = m1_buf[k][k_sub] * m2_buf[k][k_sub][j_sub];sum += mult;}}prod_buf[j_sub] = sum;}memcpy((void *)(&prod[i * 64 + j * 2]),(const void *)(&prod_buf[0]),sizeof(double ) * 2);}} */
  for (i = 0; i < 64; i++) 
...
/* Original: #pragma ACCEL TILE FACTOR=1 */
{
/* Original label: middle:for(j = 0;j < 32;j++) {#pragma ACCEL PIPELINEdouble m2_buf[32][2][64];double m1_buf[32][2];double prod_buf[2];memcpy(m1_buf[0],(const void *)(&m1[i * 64]),sizeof(double ) * 64);#pragma HLS array_partition variable=m2_buf cyclic factor = 8 dim=3memcpy_wide_bus_read_double_3d_2_64_512(m2_buf,0,0,0,(merlin_uint_512 *)m2,(j * 2 * 8),sizeof(double ) *((unsigned long )4034),4034L);for(int j_sub = 0;j_sub < 2;++j_sub) {#pragma ACCEL PARALLEL FACTOR=1i_col = i * 64;double sum =(double )0;inner:for(k = 0;k < 32;k++) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumfor(int k_sub = 0;k_sub < 2;++k_sub) {#pragma ACCEL PARALLEL#pragma ACCEL REDUCTION SCHEME=auto VARIABLE=sumk_col =(k * 2 + k_sub) * 64;mult = m1_buf[k][k_sub] * m2_buf[k][k_sub][j_sub];sum += mult;}}prod_buf[j_sub] = sum;}memcpy((void *)(&prod[i * 64 + j * 2]),(const void *)(&prod_buf[0]),sizeof(double ) * 2);} */
    double m2_buf_0[32][2][64];
    double m1_buf_0[32][2];
    double prod_buf_0[2];
    int mars_count_0_2 = 0;
    int mars_kernel_0_2_i_0 = i;
    int mars_kernel_0_2_i_1 = i;
    int mars_kernel_0_2_i_2 = i;
    double mars_kernel_0_2_m1_buf_0_0[32][2];
    
#pragma HLS array_partition variable=mars_kernel_0_2_m1_buf_0_0 complete dim=2
    double mars_kernel_0_2_m1_buf_0_1[32][2];
    
#pragma HLS array_partition variable=mars_kernel_0_2_m1_buf_0_1 complete dim=2
    double mars_kernel_0_2_m1_buf_0_2[32][2];
    
#pragma HLS array_partition variable=mars_kernel_0_2_m1_buf_0_2 complete dim=2
    double mars_kernel_0_2_m2_buf_0_0[32][2][64];
...
#pragma HLS array_partition variable=mars_kernel_0_2_m2_buf_0_0 complete dim=2
    double mars_kernel_0_2_m2_buf_0_1[32][2][64];
...
#pragma HLS array_partition variable=mars_kernel_0_2_m2_buf_0_1 complete dim=2
    double mars_kernel_0_2_m2_buf_0_2[32][2][64];
