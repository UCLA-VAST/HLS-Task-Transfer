...
static void merlin_memcpy_1(int dst[8192],int dst_idx_0,int src[4],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,src_idx_0
  ...
  unsigned long total_offset2 = 0 * 4 + src_idx_0;
  merlinL1:
  for (i = 0; i < len / 4; ++i) {
...
void mars_kernel_0_2_node_3_stage_2(int c,int exec,int r,int sol[8192],int sol_buf_0[4])
{
  
#pragma HLS INLINE OFF
  if (exec == 1) {
    merlin_memcpy_1(sol,c * 4 + r * 64,sol_buf_0,0,sizeof(int ) * ((unsigned long )4),16UL);
  }
}

void mars_kernel_0_2_bus(int mars_c,int mars_init,int mars_cond,int filter[9],int mars_filter_buf_0_0[3][3],int mars_filter_buf_0_1[3][3],int *mars_mul_1,merlin_uint_512 orig[512],int mars_orig_buf_0_0[3][64],int mars_orig_buf_0_1[3][64],int mars_r_0,int mars_r_1,int mars_r_2,int sol[8192],int mars_sol_buf_0_1[4],int mars_sol_buf_0_2[4],int *mars_temp_1)
{
...
void mars_kernel_0_2_node_2_stage_1(int c,int exec,int filter_buf_0[3][3],int *mul,int orig_buf_0[3][64],int sol_buf_0[4],int *temp)
{
...
#pragma HLS INLINE OFF
  if (exec == 1) {
    ...
    int k1;
    merlinL4:
    for (int c_sub = 0; c_sub < 4; ++c_sub) 
/* Original: #pragma ACCEL PIPELINE flatten */
/* Original: #pragma ACCEL TILE FACTOR=4 */
...
#pragma HLS pipeline
       *temp = ((int )0);
/* Original label: stencil_label3:for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEstencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;}} */
      merlinL3:
/* Original label: stencil_label3:for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEstencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;}} */
      for (k1 = 0; k1 < 3; k1++) 
...
#pragma HLS unroll
/* Original label: stencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;} */
        merlinL2:
/* Original label: stencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;} */
        for (k2 = 0; k2 < 3; k2++) 
/* Original: #pragma ACCEL PARALLEL COMPLETE */
...
#pragma HLS unroll
           *mul = filter_buf_0[k1][k2] * orig_buf_0[k1][c_sub + k2];
           *temp +=  *mul;
        }
      }
      sol_buf_0[c_sub] =  *temp;
    }
  }
}
...
#pragma ACCEL interface variable=orig depth=8192 max_depth=8192
  ...
  int temp;
  int mul;
/* Original label: stencil_label1:for(r = 0;r < 128 - 2;r++) {#pragma ACCEL PIPELINE#pragma ACCEL TILE FACTOR=1stencil_label2:for(c = 0;c < 15;c++) {#pragma ACCEL PIPELINEint orig_buf[3][64];int filter_buf[3][3];int sol_buf[4];memcpy(filter_buf[0],(const void *)(&filter[0]),sizeof(int ) * 9);#pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2memcpy_wide_bus_read_int_2d_64_512(orig_buf,0,0,(merlin_uint_512 *)orig,((c * 4 + r * 64) * 4),sizeof(int ) *((unsigned long )134),134L);for(int c_sub = 0;c_sub < 4;++c_sub) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL PARALLEL FACTOR=1temp =((int )0);stencil_label3:for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEstencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;}}sol_buf[c_sub] = temp;}memcpy((void *)(&sol[c * 4 + r * 64]),(const void *)(&sol_buf[0]),sizeof(int ) * 4);}{int orig_buf[3][64];int filter_buf[3][3];int sol_buf[2];memcpy(filter_buf[0],(const void *)(&filter[0]),sizeof(int ) * 9);#pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2memcpy_wide_bus_read_int_2d_64_512(orig_buf,0,0,(merlin_uint_512 *)orig,((60 + r * 64) * 4),sizeof(int ) *((unsigned long )132),132L);for(c = 60L;c < 64 - 2;c++) {#pragma ACCEL PIPELINEtemp =((int )0);for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEfor(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][- 60 + c + k2];temp += mul;}}sol_buf[- 60 + c] = temp;}memcpy((void *)(&sol[60 + r * 64]),(const void *)(&sol_buf[0]),sizeof(int ) * 2);}} */
  merlinL11:
/* Original label: stencil_label1:for(r = 0;r < 128 - 2;r++) {#pragma ACCEL PIPELINE#pragma ACCEL TILE FACTOR=1stencil_label2:for(c = 0;c < 15;c++) {#pragma ACCEL PIPELINEint orig_buf[3][64];int filter_buf[3][3];int sol_buf[4];memcpy(filter_buf[0],(const void *)(&filter[0]),sizeof(int ) * 9);#pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2memcpy_wide_bus_read_int_2d_64_512(orig_buf,0,0,(merlin_uint_512 *)orig,((c * 4 + r * 64) * 4),sizeof(int ) *((unsigned long )134),134L);for(int c_sub = 0;c_sub < 4;++c_sub) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL PARALLEL FACTOR=1temp =((int )0);stencil_label3:for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEstencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;}}sol_buf[c_sub] = temp;}memcpy((void *)(&sol[c * 4 + r * 64]),(const void *)(&sol_buf[0]),sizeof(int ) * 4);}{int orig_buf[3][64];int filter_buf[3][3];int sol_buf[2];memcpy(filter_buf[0],(const void *)(&filter[0]),sizeof(int ) * 9);#pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2memcpy_wide_bus_read_int_2d_64_512(orig_buf,0,0,(merlin_uint_512 *)orig,((60 + r * 64) * 4),sizeof(int ) *((unsigned long )132),132L);for(c = 60L;c < 64 - 2;c++) {#pragma ACCEL PIPELINEtemp =((int )0);for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEfor(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][- 60 + c + k2];temp += mul;}}sol_buf[- 60 + c] = temp;}memcpy((void *)(&sol[60 + r * 64]),(const void *)(&sol_buf[0]),sizeof(int ) * 2);}} */
  for (r = 0; r < 128 - 2; r++) 
/* Original: #pragma ACCEL PIPELINE */
...
/* Original: #pragma ACCEL TILE FACTOR=1 */
{
/* Original label: stencil_label2:for(c = 0;c < 15;c++) {#pragma ACCEL PIPELINEint orig_buf[3][64];int filter_buf[3][3];int sol_buf[4];memcpy(filter_buf[0],(const void *)(&filter[0]),sizeof(int ) * 9);#pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2memcpy_wide_bus_read_int_2d_64_512(orig_buf,0,0,(merlin_uint_512 *)orig,((c * 4 + r * 64) * 4),sizeof(int ) *((unsigned long )134),134L);for(int c_sub = 0;c_sub < 4;++c_sub) {#pragma ACCEL PIPELINE AUTO#pragma ACCEL PARALLEL FACTOR=1temp =((int )0);stencil_label3:for(k1 = 0;k1 < 3;k1++) {#pragma ACCEL PARALLEL COMPLETEstencil_label4:for(k2 = 0;k2 < 3;k2++) {#pragma ACCEL PARALLEL COMPLETEmul = filter_buf[k1][k2] * orig_buf[k1][c_sub + k2];temp += mul;}}sol_buf[c_sub] = temp;}memcpy((void *)(&sol[c * 4 + r * 64]),(const void *)(&sol_buf[0]),sizeof(int ) * 4);} */
    int orig_buf_0[3][64];
...
#pragma HLS array_partition variable=mars_kernel_0_2_orig_buf_0_2 complete dim=1
    ...
    int mars_kernel_0_2_sol_buf_0_0[4];
    int mars_kernel_0_2_sol_buf_0_1[4];
    int mars_kernel_0_2_sol_buf_0_2[4];
    merlinL10:
    for (c = 0; c < 15 + 2; c++) 
/* Original: #pragma ACCEL PIPELINE flatten */
/* Original: #pragma ACCEL TILE FACTOR=4 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=orig_buf cyclic factor = 16 dim=2 */
      memcpy_wide_bus_read_int_2d_64_512(orig_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)orig,(size_t )((60 + r * 64) * 4),sizeof(int ) * ((unsigned long )132),(size_t )132L);
      merlinL9:
