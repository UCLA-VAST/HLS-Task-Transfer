...
/* Original label: loopkk:for(kk = 0;kk <= 7;kk++) {#pragma ACCEL PIPELINE#pragma ACCEL TILE FACTOR=1int _in_kk =(int )(((long )0) + 8L *((long )kk));loopi:for(i = 0;i < 64;++i) {#pragma ACCEL PARALLEL COMPLETEloopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}}}} */
      merlinL3:
/* Standardize from: for(kk = 0;kk < 64;kk += 8) {...} */
/* Original label: loopkk:for(kk = 0;kk <= 7;kk++) {#pragma ACCEL PIPELINE#pragma ACCEL TILE FACTOR=1int _in_kk =(int )(((long )0) + 8L *((long )kk));loopi:for(i = 0;i < 64;++i) {#pragma ACCEL PARALLEL COMPLETEloopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}}}} */
      for (kk = 0; kk <= 7; kk++) 
...
/* Original label: loopi:for(i = 0;i < 64;++i) {#pragma ACCEL PARALLEL COMPLETEloopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}}} */
        merlinL2:
/* Original label: loopi:for(i = 0;i < 64;++i) {#pragma ACCEL PARALLEL COMPLETEloopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}}} */
        for (i = 0; i < 64; ++i) 
...
#pragma HLS unroll
/* Original label: loopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}} */
          merlinL1:
/* Original label: loopk:for(k = 0;k < 8;++k) {#pragma ACCEL PARALLEL COMPLETEi_row = i * 64;k_row =(k + _in_kk) * 64;temp_x = m1_buf[i][kk][k];loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;}} */
          for (k = 0; k < 8; ++k) 
/* Original: #pragma ACCEL PIPELINE off */
...
/* Original label: loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;} */
            merlinL0:
/* Original label: loopj:for(j = 0;j < 8;++j) {#pragma ACCEL PARALLEL COMPLETEmul = temp_x * m2_buf[kk][k][jj][j];prod_buf[i][jj][j] += mul;} */
            for (j = 0; j < 8; ++j) 
/* Original: #pragma ACCEL PARALLEL COMPLETE */
