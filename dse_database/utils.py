...
def plot_points_with_subplot(points_dict_multi_target, label, save_dir, target_list, use_sigma=False):
    ...
    for idx, target in enumerate(target_list):
        ...
        for pname, points in points_dict.items():
            # if target == 'perf':
            #     xs = [(1e7/math.log2(point[0]))/2 for point in sorted(points)]
            #     ys = [(1e7/math.log2(point[1]))/2 for point in sorted(points)]
            # else:
        ...
        #         xs = [point[0] for point in sorted(points)]
        #         ys = [point[1] for point in sorted(points)]
        #     plt.plot(range(len(xs)), xs, POINTS_MARKERS[i % len(POINTS_MARKERS)],
        #             color=POINTS_COLORS[i % len(POINTS_COLORS)],
        #             label=f'SDx18.3')
        #     i += 1    
        #     plt.plot(range(len(ys)), ys, POINTS_MARKERS[i % len(POINTS_MARKERS)],
        #             color=POINTS_COLORS[i % len(POINTS_COLORS)],
        #             label=f'Vitis20.2')
        #     i += 1  
        plt.legend(loc='best')
