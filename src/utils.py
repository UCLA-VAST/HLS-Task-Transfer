...
def get_save_path():
    ...
    return '/home/zijd/scratch/dse/save'
...
class MLP_multi_objective(nn.Module):
    ...
    def forward(self, x, return_last=False):
        ...
        out_MLP: Dict[str, torch.Tensor] = {}
        out_last: Dict[str, torch.Tensor] = dict()
        for obj in self.objectives:
            ...
            for layer_ind, layer in enumerate(self.MLP_heads[obj]):
                if layer_ind + self.num_common_lyr == len(self.layer_channels) - 1:
                    ...
                    if layer_ind == len(self.MLP_heads[obj])-1:
                        out_last[obj] = torch.clone(out_MLP[obj])
                    out_MLP[obj] = self.activation(layer(out_MLP[obj]))
        # model.store_layer_output(self, layer_inputs[-1])
        if return_last:
            return out_MLP, out_last
        return out_MLP
